...
const statusColors = { 0: 0x808080, 1: 0xff4444, 2: 0xff9800, 3: 0xffeb3b, 4: 0x4CAF50 };
...
const dispenserPositions = [
    { id: 1, x: -1.5, y: 2.0, z: 4, rotation: 0, scale: 0.25 },
    { id: 2, x: -4.8, y: 2.0, z: -8, rotation: 90, scale: 0.25 },
    { id: 3, x: -4.8, y: 2.0, z: -16, rotation: 90, scale: 0.25 },
    { id: 4, x: -4.8, y: 3.80, z: -8, rotation: 90, scale: 0.25 },
    { id: 5, x: -4.8, y: 3.80, z: -16, rotation: 90, scale: 0.25 },
    { id: 6, x: -4.8, y: 3.80, z: 14, rotation: 90, scale: 0.25 },
    { id: 7, x: -4.8, y: 3.80, z: 21.5, rotation: 90, scale: 0.25 },
    { id: 8, x: -4.8, y: 5.50, z: -8, rotation: 90, scale: 0.25 },
    { id: 9, x: -4.8, y: 5.50, z: -16, rotation: 90, scale: 0.25 },
    { id: 10, x: -4.8, y: 5.50, z: 14, rotation: 90, scale: 0.25 },
    { id: 11, x: -4.8, y: 5.50, z: 21.5, rotation: 90, scale: 0.25 },
    { id: 12, x: -4.8, y: 7.30, z: 17.5, rotation: 90, scale: 0.25 },
    { id: 13, x: -10, y: 7.30, z: 17.5, rotation: 270, scale: 0.25 }
];
...
function loadDispenserModel() {
    ...
    loader.load('NEW DSP.glb', function(gltf) {
        dispenserPositions.forEach((pos, index) => {
            ...
            dispenser.userData.dispenserId = `DSP_${pos.id}`;
            dispenser.traverse(function(node) {
                if (node.isMesh) {
                    ...
                    if (nodeName.includes('pump') || nodeName.includes('cap')) {
                        ...
                        node.material = new THREE.MeshStandardMaterial({ color: statusColors[0], roughness: 0.3, metalness: 0.1, emissive: statusColors[0], emissiveIntensity: 0.1 });
                    }
                }
            });
            scene.add(dispenser);
...
function checkLoadingComplete() {
    if (modelsLoaded.building && modelsLoaded.tiles && modelsLoaded.lights && modelsLoaded.doors && modelsLoaded.windows && modelsLoaded.columns && modelsLoaded.dispensers) {
        document.getElementById('loading-3d').style.display = 'none';
        document.getElementById('loading').style.display = 'none';
        sceneReady = true;
        
        Object.entries(pendingStatusUpdates).forEach(([id, s]) => {
            updateDispenser3DStatus(parseInt(id), s);
        });
        pendingStatusUpdates = {};
        
        Object.entries(allDispenserStatuses).forEach(([dispId, s]) => {
            const match = dispId.match(/DSP_(\d+)/i);
            if (match) updateDispenser3DStatus(parseInt(match[1]), s);
        });
    }
}

function updateDispenser3DStatus(localId, statusValue) {
    if (!sceneReady) {
        pendingStatusUpdates[localId] = statusValue;
        return;
    }
    const dispenser = dispensers3D.find(d => d.userData.id === localId);
    if (!dispenser) return;
    dispenser.traverse(node => {
        if (node.isMesh) {
            const nodeName = node.name.toLowerCase();
            if (!nodeName.includes('pump') && !nodeName.includes('cap')) {
                node.material.color.setHex(statusColors[statusValue]);
                node.material.emissive.setHex(statusColors[statusValue]);
                node.material.emissiveIntensity = statusValue === 1 ? 0.4 : 0.15;
            }
        }
    });
}
...
function setup3DControls() {
    ...
    canvas.addEventListener('wheel', (e) => { e.preventDefault(); slideZ(-(e.deltaX || e.deltaY) * 0.02); }, { passive: false });
...
function loadDashboardData() {
    function effectiveStatus(id) {
        ...
        return hb && (Date.now() - hb <= 70000) ? (trueFirebaseStatus[id] ?? 0) : 0;
    }
    ...
    db.ref("dispenser").on("value", snap => {
        ...
        Object.keys(trueFirebaseStatus).forEach(id => { allDispenserStatuses[id] = effectiveStatus(id); });
        renderStatusCards();
    ...
    db.ref("heartbeat").on("value", snap => {
        const data = snap.val() || {};
        Object.keys(data).forEach(id => { const d = data[id]; if (d && d.epoch) lastHeartbeat[id] = d.epoch; });
